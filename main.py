...
def countUndefinedCells(cellArray):
    counter = 0
    for i in cellArray:
        if i == strings.SYMBOL_UNDEF:
            counter += 1
    return counter


def game(callBackData):
    message = strings.ANSW_YOUR_TURN
    alert = None
    ...
    if not buttonNumber == 9:
        ...
        charList.pop(0)
        if charList[
            buttonNumber] == strings.SYMBOL_UNDEF:
            charList[buttonNumber] = strings.SYMBOL_X
            if isWin(charList, strings.SYMBOL_X):
                message = strings.ANSW_YOU_WIN
            else:
                if countUndefinedCells(charList) != 0:
                    isCycleContinue = True
                    while (isCycleContinue):
                        rand = random.randint(0, 8)
                        if charList[rand] == strings.SYMBOL_UNDEF:
                            charList[rand] = strings.SYMBOL_O
                            isCycleContinue = False
                            if isWin(charList, strings.SYMBOL_O):
                                message = strings.ANSW_BOT_WIN
        else:
            alert = strings.ALERT_CANNOT_MOVE_TO_THIS_CELL

        if countUndefinedCells(charList) == 0 and message == strings.ANSW_YOUR_TURN:
            message = strings.ANSW_DRAW

        callBackData = ''
        for c in charList:
            callBackData += c

    if message == strings.ANSW_YOU_WIN or message == strings.ANSW_BOT_WIN or message == strings.ANSW_DRAW:
        message += '\n'
        for i in range(0, 3):
            message += '\n | '
            for j in range(0, 3):
...
def newGame(update, _):
    data = ''
    for i in range(0, 9):
        data += strings.SYMBOL_UNDEF
    update.message.reply_text(strings.ANSW_YOUR_TURN, reply_markup=InlineKeyboardMarkup(getKeyboard(data)))
...
if __name__ == '__main__':
    updater = Updater('5685822105:AAE7Dp2Ewy9_8A2mRyyaWBHQBp5enWg2xPU')
